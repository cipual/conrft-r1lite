...
class SACAgentSingleArm(flax.struct.PyTreeNode):
    """
    ...
    def critic_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        rewards = batch["rewards"] + batch["grasp_penalty"]
        ...
        target_q = (rewards + self.config["discount"] * batch["masks"] * target_next_min_q)
        ...
        predicted_qs = self.forward_critic(batch["observations"], actions, rng=rng, grad_params=params)
    ...
    def policy_bc_loss_fn(self, batch, params: Params, rng: PRNGKey):
        rng, policy_rng = jax.random.split(rng)
        temperature = self.forward_temperature()
        
        action_distributions = self.forward_policy(batch["observations"], rng=policy_rng, grad_params=params)
        pi_actions = action_distributions.mode()
    ...
    def update(
        ...
        **kwargs
        ...
        assert networks_to_update.issubset(loss_fns.keys()), f"Invalid gradient steps: {networks_to_update}"
