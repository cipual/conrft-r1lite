...
def _plot_trajectories_3d(recorded_by_arm: Dict[str, np.ndarray], joint_by_arm: Dict[str, np.ndarray], pose_by_arm: Dict[str, np.ndarray], output_path: Path):
    ...
    for arm, recorded in recorded_by_arm.items():
        ...
        joint_actual = joint_by_arm[arm]
        pose_actual = pose_by_arm[arm]
        ax.plot(recorded[:, 0], recorded[:, 1], recorded[:, 2], label=f"{arm} recorded", color=rec_color, linewidth=2.2)
        ax.plot(joint_actual[:, 0], joint_actual[:, 1], joint_actual[:, 2], label=f"{arm} joint action replay", color=action_color, linewidth=1.8)
...
def _write_csv(
    ...
    joint_pose_history: Dict[str, np.ndarray],
    joint_joint_history: Dict[str, np.ndarray],
    joint_gripper_history: Dict[str, np.ndarray],
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for idx, transition in enumerate(trajectory):
            for arm in arms:
                obs = _obs_for_arm(transition, arm, "observations")
                ...
                joint_actual = np.asarray(joint_pose_history[arm][idx], dtype=np.float32)
                pose_actual = np.asarray(pose_pose_history[arm][idx], dtype=np.float32)
                joint_gripper = float(joint_gripper_history[arm][idx])
                pose_gripper = float(pose_gripper_history[arm][idx])
                joint_delta = np.asarray(
                    action.get(
                        "joint_delta",
                        np.asarray(next_obs["joint_pos"], dtype=np.float32)
                        - np.asarray(obs["joint_pos"], dtype=np.float32),
                    ),
                    dtype=np.float32,
                )
                target_joint = (np.asarray(obs["joint_pos"], dtype=np.float32) + joint_delta).astype(np.float32)
                actual_joint = np.asarray(joint_joint_history[arm][idx], dtype=np.float32)
                ...
                writer.writerow(
                    [
                        ...
                        *joint_actual.tolist(),
                        ...
                        recorded_gripper,
                        joint_gripper,
                        abs(joint_gripper - recorded_gripper),
                        *pose_actual.tolist(),
...
def _run_replay(
    ...
    actual_joint_history: Dict[str, List[np.ndarray]] = {item: [] for item in arms}
    ...
    for idx, transition in enumerate(trajectory):
        ...
        if replay_mode == "joint_action":
            ...
            for item in arms:
                ...
                current_joint = np.asarray(state["state"][item]["joint_pos"], dtype=np.float32)
                current_gripper = np.asarray(state["state"][item]["gripper_pose"], dtype=np.float32)
                joint_delta = np.asarray(
                    action.get(
                        "joint_delta",
                        np.asarray(_obs_for_arm(transition, item, "next_observations")["joint_pos"], dtype=np.float32)
                        - np.asarray(_obs_for_arm(transition, item, "observations")["joint_pos"], dtype=np.float32),
                    ),
                    dtype=np.float32,
                )
                joint_delta_norms.append(float(np.linalg.norm(joint_delta)))
        ...
        for item in arms:
            ...
            actual_joint = np.asarray(raw["state"][item]["joint_pos"], dtype=np.float32)
            ...
            actual_joint_history[item].append(actual_joint)
        ...
        if idx % max(1, log_every) == 0 or idx == len(trajectory) - 1:
            extra = ""
            if replay_mode == "joint_action":
                extra = " joint_delta_norms=" + ",".join(f"{v:.4f}" for v in joint_delta_norms)
            print(
    ...
    return (
        ...
        {item: np.asarray(values, dtype=np.float32) for item, values in actual_joint_history.items()},
...
def main():
    ...
    try:
        joint_pose_arr, joint_joint_arr, joint_gripper_arr = _run_replay(
            client=client,
        ...
        pose_pose_arr, _, pose_gripper_arr = _run_replay(
            client=client,
            trajectory=trajectory,
            arm=args.arm,
            server_mode=args.mode,
    ...
    _plot_trajectories_3d(recorded, joint_pose_arr, pose_pose_arr, output_image_3d)
    _write_csv(trajectory, joint_pose_arr, joint_joint_arr, joint_gripper_arr, pose_pose_arr, pose_gripper_arr, arms, output_csv)
    output_npz.parent.mkdir(parents=True, exist_ok=True)
    np.savez(
        output_npz,
        **{f"{arm}_recorded": arr for arm, arr in recorded.items()},
        **{f"{arm}_joint_action_actual_pose": arr for arm, arr in joint_pose_arr.items()},
        **{f"{arm}_joint_action_actual_joint": arr for arm, arr in joint_joint_arr.items()},
        **{f"{arm}_joint_action_actual_gripper": arr for arm, arr in joint_gripper_arr.items()},
